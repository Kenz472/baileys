...
const executeWMexQuery = async (
    ...
    if (child?.content) {
        ...
        if (data.errors && data.errors.length > 0) {
            ...
            throw new boom_1.Boom('GraphQL server error:' + errorMessages, {
    ...
    throw new boom_1.Boom(`Failed to ${action}, unexpected response structure.`, {
        statusCode: 400,
        data: result
    });
};
        ...
        newsletterFromUrl: async (url) => {
            try {
                ...
                if (!resultNode?.content) {
                    throw new boom_1.Boom('No result content in response', {
                        statusCode: 400,
                        data: result
                    });
                }
                ...
                if (!parsedResult?.data) {
                    throw new boom_1.Boom('No data field in response', {
                        statusCode: 400,
                        data: parsedResult
                    });
                }
                ...
                if (!metadataPath) {
                    throw new boom_1.Boom('Newsletter not found or access denied', {
                        statusCode: 404,
                ...
                throw new boom_1.Boom(`Failed to fetch newsletter from URL: ${error.message}`, {
