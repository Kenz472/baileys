...
const makeSocket = (config) => {
    ...
    const createWebSocket = () => {
        const wsClient = new Client_1.WebSocketClient(url, {
            ...config,
            connectTimeoutMs: Math.min(connectTimeoutMs || 20000, 60000),
            handshakeTimeout: Math.min(connectTimeoutMs || 20000, 60000)
        });
        return wsClient;
    };
    ...
    const waitForMessage = async (msgId, timeoutMs) => {
        const effectiveTimeout = timeoutMs ?? defaultQueryTimeoutMs;

        if (!effectiveTimeout) {
            throw new boom_1.Boom('No timeout specified for waitForMessage', { statusCode: 400 });
        }
        ...
        try {
            const result = await (0, Utils_1.promiseTimeout)(effectiveTimeout, (resolve, reject) => {
                onRecv = resolve;
                onErr = err => {
    ...
    const uploadPreKeysToServerIfRequired = async (force = false) => {
        try {
            ...
            logger.warn({ err }, 'failed to check/upload pre-keys');
        }
    };
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        if (
            ...
            message.includes('rate-limit') ||
            message.includes('overlimit') ||
            message.includes('over-limit')
        ) {
            rateLimitDelay = Math.min(rateLimitDelay + 5000, 60000);
            logger.info({ rateLimitDelay }, 'rate limit detected, increasing delay');

            setTimeout(() => {
                rateLimitDelay = Math.max(rateLimitDelay - 5000, 0);
                logger.info({ rateLimitDelay }, 'reducing rate limit delay');
            }, 30000);
        }
    };
    ...
    function end(error) {
        ...
        const isTimeoutError =
            errorMessage.includes('opening handshake has time out') ||
            errorMessage.includes('WebSocket error') ||
            errorMessage.includes('408') ||
            errorMessage.includes('ETIMEDOUT') ||
            errorMessage.includes('ECONNRESET');

        if (isTimeoutError && reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            logger.info('WebSocket timeout detected, attempting reconnect...');
            closed = false;
    ...
    const handleReconnect = async (error) => {
...
function mapWebSocketError(handler) {
    return (error) => {
        const statusCode = (0, Utils_1.getCodeFromWSError)(error);
        ...
        if (
            ...
            handler(new boom_1.Boom('WebSocket handshake timeout', {
                statusCode: 408,
                data: error
            }));
        } else {
            handler(new boom_1.Boom(`WebSocket Error (${errorMessage})`, {
