...
async function fetchLatestWaWebVersion() {
    try {
        const response = await axios_1.default.get('https://wppconnect.io/whatsapp-versions/');
        const $ = cheerio_1.load(response.data);

        const subtitle = $('.hero__subtitle').text().trim();
        const rawVersionText = $('.buttons__UCS a').text().trim();
        const versionLink = $('.buttons__UCS a').attr('href');

        const version = rawVersionText.replace(/[^0-9.]/g, ''); 
        let hasil = String(version)
        .replace(/[^0-9.]/g, '') 
        .replace(/\./g, ',')    
        return {
            success: true,
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/Kenz472/baileys/master/lib/Defaults/baileys-version.json';
    try {
